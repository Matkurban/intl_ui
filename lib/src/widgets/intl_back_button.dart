...
class IntlBackButton extends StatelessWidget {
  const IntlBackButton({
    ...
    this.color,
    this.isLtr = true,
    this.ltrLeadingIcon = Icons.arrow_back,
    this.rtlLeadingIcon = Icons.arrow_forward,
    this.size,
    this.onPressed,
  });
  ...
  /// 左向图标（用于 LTR 布局）。
  final IconData ltrLeadingIcon;

  /// The icon for the right direction (used in RTL layout).
  /// 右向图标（用于 RTL 布局）。
  final IconData rtlLeadingIcon;
  ...
  Widget build(BuildContext context) {
    return IconButton(
      ...
      // 使用提供的 onPressed 回调函数，若未提供则默认执行 Navigator.maybePop。
