...
class DioLoggerInterceptors extends Interceptor {
  final DioLoggerScreenLogic loggerLogic = DioLoggerScreenLogic.instance; // 使用单例
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    ...
    loggerLogic.addLog(DioLogModel(request: options, timestamp: now)); // 使用 addLog 方法
  ...
  void onResponse(Response response, ResponseInterceptorHandler handler) {
    ...
    if (index != -1) {
      ...
      loggerLogic.updateLog(
        index,
        loggerLogic.logList[index].copyWith(response: response, duration: duration),
      ); // 使用 updateLog 方法
    }
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) {
    ...
    if (index != -1) {
      ...
      loggerLogic.updateLog(
        index,
        loggerLogic.logList[index].copyWith(response: err.response, error: err, duration: duration),
      ); // 使用 updateLog 方法
    }
